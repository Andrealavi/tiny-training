...
def main():
    ...
    model = build_mcu_model().to_device("mps")
    ...
    if configs.evaluate:
        ...
        with open(f"./tests/test_e_{str(configs.run_config.warmup_epochs)}_d_{str(configs.data_provider.dataset)}b_{str(configs.backward_config.n_bias_update)}_{str(configs.backward_config.manual_bias_idx)}_w_{str(configs.backward_config.manual_weight_idx)}_r_{str(configs.backward_config.weight_update_ratio)}_{time()}.txt", "w") as f:
            ...
            f.write("\n")
            f.write("Validations at different epochs: \n")
            for e, val in trainer.training_validations.items():
                f.write(str(e))
                f.write(": ")
                f.write(str(val))
                f.write("\n")
